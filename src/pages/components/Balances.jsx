...
function Balances({ split, documentID, setSplit }) {
  let { balances, userMapping } = split;
  ...
  function getMin(bal) {
    ...
    Object.keys(bal).forEach((person) => {
      if (bal[person] < min) min = bal[person];
    });
  ...
  function getMax(bal) {
    ...
    Object.keys(bal).forEach((person) => {
      if (bal[person] > max) max = bal[person];
    });
  ...
  const getSettlementArray = () => {
    ...
    while (1) {
      ...
      if (balance[min] === 0 || balance[max] === 0) break;
      if (Math.abs(balance[min]) <= balance[max]) {
        ...
        balance[max] = parseFloat(
          (balance[max] - Math.abs(balance[min])).toFixed(2)
        );
        balance[min] = 0;
  ...
  const markBalanceAsPaid = async () => {
    ...
    split.balances[markAsPaid.from] = temp
      .plus(new Decimal(Number(markAsPaid.amount).toFixed(2)))
      .toNumber();
    split.balances[markAsPaid.to] = temp1
      .minus(new Decimal(Number(markAsPaid.amount).toFixed(2)))
      .toNumber();
    try {
  ...
  const getTotalPaidBy = (friend) => {
    let sum = 0;
    split.expenses?.forEach((expense) => {
      if (expense.paidBy === friend) sum += Number(expense.amount);
    });
    split.reimbursement?.forEach((expense) => {
      if (expense.from === friend) sum += Number(expense.amount);
      if (expense.to === friend) sum -= Number(expense.amount);
    });
    return formatMoney(sum);
  };
          ...
          {split.reimbursement.map((reimbursement) => {
            return (
              <div className="my-3" key={Math.random()}>
                <div
                  className="card flex-row align-items-center justify-content-between w-100 px-2 d-flex"
                  style={{}}
                >
                  <div className="my-2" style={{ display: "inline" }}>
                    <img
                      src={`https://api.dicebear.com/6.x/adventurer/svg?scale=100&skinColor=9e5622,f2d3b1,ecad80&seed=${reimbursement.from}&radius=40&size=64`}
                      alt=""
                    />
                  ...
                  <div>
                    ...
                    <img
                      src={`https://api.dicebear.com/6.x/adventurer/svg?scale=100&skinColor=9e5622,f2d3b1,ecad80&seed=${reimbursement.to}&radius=40&size=64`}
                      alt=""
                    />
                  </div>
                </div>
  ...
  return (
    <>
      {split && (
        <div
          className="w-100 p-2 d-flex lin-grad1 flex-column justify-content-center"
        ...
        >
          ...
          <div
            className="bold-font d-flex align-items-center"
            style={{ fontSize: "3rem" }}
          >
        ...
        <>
          ...
          >
            <div
              ...
              style={{ backgroundColor: "#252525", borderRadius: "10px" }}
            >
              <div className="p-3">
                <h5 className="text-center mont mb-3">
                  Are you sure{" "}
                  <span style={{ color: "#f27979" }}>
                    {userMapping[markAsPaid.from] || markAsPaid.from}
                  </span>{" "}
                  has paid{" "}
                  <span style={{ color: "#67e9a9" }}>
                    {userMapping[markAsPaid.to] || markAsPaid.to}
                  </span>{" "}
                  , {formatMoney(markAsPaid.amount)}{" "}
      ...
      <div className="my-4">
        {split && (
          <table
            className="table table-sm table-dark table-borderless table-striped"
                  ...
                  <tr key={Math.random()}>
                    ...
                    >
                      <GoPrimitiveDot
                        style={{
                          color:
                            split.balances[friend] >= 0 ? "#67e9a9" : "#f27979",
                        }}
                      />
                      {userMapping[friend] || friend}
                    </td>
                    <td className="text-center d-flex align-items-center justify-content-center">
                      {formatMoney(getTotalPaidBy(friend))}
                    ...
                    >
                      <div className="d-flex align-items-center justify-content-center">
                        ...
                        {formatMoney(Math.abs(split.balances[friend]))}
                        <span style={{ marginLeft: "4px", fontSize: "1rem" }}>
                          🐟
                        </span>
                      </div>
      ...
      <div>
        <div className="d-flex justify-content-between">
          ...
          <p
            onClick={() => {
              setShowReimbursementHitory(true);
            }}
            style={{ color: "#9ec0e5", textDecoration: "underline" }}
          >
          ...
          return (
            <div className="my-2" key={Math.random()}>
              ...
              >
                <div className="my-2" style={{ display: "inline" }}>
                  <img
                    src={`https://api.dicebear.com/6.x/adventurer/svg?scale=100&skinColor=9e5622,f2d3b1,ecad80&seed=${settlement.from}&radius=40&size=80`}
                    alt=""
                  />
                  {/* <img src={`https://api.dicebear.com/6.x/big-ears-neutral/svg?mouth=variant0704,variant0705,variant0501&seed=${settlement.from}&radius=40&size=48`} alt="" /> */}
                </div>
                <div className="d-flex justify-content-center align-items-center flex-column">
                  <span>{userMapping[settlement.from] || settlement.from}</span>
                  <span
                    style={{ fontSize: "0.8rem", color: "#f27979" }}
                ...
                <div>{userMapping[settlement.to] || settlement.to}</div>
                <div>
                  {/* <img src={`https://api.dicebear.com/6.x/big-ears-neutral/svg?mouth=variant0707,variant0702,variant0201&seed=${settlement.to}&radius=40&size=48`} alt="" /> */}
                  <img
                    src={`https://api.dicebear.com/6.x/adventurer/svg?scale=100&skinColor=9e5622,f2d3b1,ecad80&seed=${settlement.to}&radius=40&size=80`}
                    alt=""
                  />
                </div>
              </div>
              <div
                className="w-100 py-1 d-flex justify-content-center"
                onClick={() => {
